...
## Временный контракт по идентификации студента

- Пока в бизнес-логике поиск студента выполняется по `Id` (`Guid`).
- Поле `StudentNumber` оставлено в схеме как временное и сейчас в логике не используется.
- Поле `ExternalStudentNumberRaw` добавлено только для хранения внешнего номера "как пришел",
  формат этого значения пока не утвержден.